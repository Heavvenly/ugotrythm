...
function showSongInfo(songName) {
  ...
  const rank = songRanks[data.songName];
  ...
  document.getElementById("rank-info").textContent = `Best Rank: ${rank}` || "D";
  document.getElementById("extraNotes").textContent = data.notesInfo || "No extra info";

  const difficulty = data.difficultyValue || 0;
  const bar = document.getElementById("difficulty-bar");
  bar.style.width = `${difficulty}%`;

  document.getElementById("play-song-button").onclick = () => startGame(songName);
...
function startGame(songName) {
  ...
  document.getElementById("song-title").textContent = `${songtitle} by ${artist}`;
...
function updateRankProgressDisplay() {
  ...
  document.getElementById("rankAProgress").textContent = `Songs with at least A Rank: ${countA}/12`;
  document.getElementById("rankSProgress").textContent = `Songs with at least S Rank: ${countS}/12`;
  document.getElementById("rankSPlusProgress").textContent = `Songs with S+ Rank: ${countSPlus}/12`;
